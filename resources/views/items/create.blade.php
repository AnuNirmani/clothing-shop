<x-app-layout>
    ...
    <style>
        .create-root { font-family: 'DM Sans', sans-serif; }
        ...
        /* ── Select: placeholder state (matches "Select colors…" muted lavender) ── */
        select.field-input {
            appearance: none;
            ...
            padding-right: 36px;
            cursor: pointer;
            /* Default = muted lavender placeholder colour */
            color: #c4b5d4;
            font-size: 13px;
        }
        /* Once a real value is chosen, text goes dark */
        select.field-input.has-value {
            color: #1f2937;
            font-size: 13.5px;
        }
        /* Options inside the dropdown are always dark */
        select.field-input option {
            color: #1f2937;
            font-size: 13.5px;
            font-family: 'DM Sans', sans-serif;
        }
        /* The empty placeholder option stays muted */
        ...
        .toggle-row-label { font-size: 13.5px; font-weight: 600; color: #374151; cursor: pointer; }
    ...
    <div class="create-root" style="background: linear-gradient(135deg,#fdf2f8 0%,#f0f9ff 55%,#fdf4ff 100%); min-height: calc(100vh - 64px); padding: 28px 16px 60px;">
        <div style="max-width: 1200px; margin: 0 auto;">

            {{-- ── Error Alert ── --}}
            @if ($errors->any())
            <div class="error-alert fade-up d1 mb-6">
            ...
            {{-- ── Hero bar ── --}}
            <div class="fade-up d1 mb-6 relative rounded-3xl overflow-hidden p-6 md:p-8" style="background: linear-gradient(135deg,#130826,#1e0d4a,#0a1628); box-shadow: 0 20px 60px rgba(13,5,32,0.3);">
                <div style="position:absolute;top:-40px;right:-40px;width:200px;height:200px;background:radial-gradient(circle,rgba(244,114,182,0.2) 0%,transparent 70%);pointer-events:none;"></div>
                <div style="position:absolute;bottom:-30px;left:40%;width:160px;height:160px;background:radial-gradient(circle,rgba(96,165,250,0.15) 0%,transparent 70%);pointer-events:none;"></div>
                <div style="position:relative;z-index:1;display:flex;align-items:center;gap:20px;">
                    <div style="width:52px;height:52px;border-radius:16px;background:linear-gradient(135deg,rgba(244,114,182,0.25),rgba(96,165,250,0.2));border:1px solid rgba(244,114,182,0.3);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <svg class="w-6 h-6" style="color:#f9a8d4;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                        </svg>
                    </div>
                    <div>
                        <p style="color:rgba(249,168,212,0.6);font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.3em;margin-bottom:4px;">✦ Inventory · New Product</p>
                        <h1 class="font-display" style="color:white;font-size:22px;font-weight:700;margin:0;">Create a New Item</h1>
                        <p style="color:rgba(255,255,255,0.35);font-size:12.5px;margin-top:3px;">Fill in the details below to add a product to your catalogue</p>
                    </div>
                </div>
            </div>
            ...
            <form action="{{ route('items.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div style="display:grid;grid-template-columns:1fr;gap:20px;" class="lg-grid">
                    <style>
                        @media(min-width:1024px){ .lg-grid { grid-template-columns: 1fr 340px !important; } }
                    ...
                    <div style="display:flex;flex-direction:column;gap:20px;">
                        ...
                        <div class="form-card card-pink fade-up d2">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                                <div>
                                    ...
                                    <input type="text" name="name" value="{{ old('name') }}" placeholder="e.g. Silk Wrap Dress" class="field-input">
                                </div>
                                <div>
                                    <label class="field-label">Co. Name <span class="req">*</span></label>
                                    <input type="text" name="co_name" value="{{ old('co_name') }}" placeholder="Brand or company name" class="field-input">
                                </div>
                                <div>
                                    <label class="field-label">SKU <span class="req">*</span></label>
                                    <input type="text" name="SKU" value="{{ old('SKU') }}" placeholder="e.g. DRS-001-BLK" class="field-input">
                                </div>
                                <div>
                                    <label class="field-label">Stock Quantity <span class="req">*</span></label>
                                    <input type="number" name="stock_items" value="{{ old('stock_items') }}" placeholder="0" min="0" class="field-input">
                                </div>
                            </div>

                            <div style="margin-top:16px;">
                                <label class="field-label">Description</label>
                                <textarea name="description" placeholder="Describe the product in detail…" class="field-input">{{ old('description') }}</textarea>
                            </div>
                            <div style="margin-top:14px;">
                                <label class="field-label">Internal Note</label>
                                <textarea name="note" rows="2" placeholder="Any internal notes (not visible to customers)" class="field-input" style="min-height:70px;">{{ old('note') }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="form-card card-blue fade-up d3">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                                ...
                                <div>
                                    ...
                                    <select name="type_id" id="type_id" class="field-input" required disabled>
                                        <option value="">{{ old('category_id') ? 'Select type…' : 'Select category first…' }}</option>
                                    ...
                                    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:4px;max-height:160px;overflow-y:auto;border:1.5px solid #e0e7ff;border-radius:14px;padding:10px;background:linear-gradient(135deg,#f0f4ff,#fdf9ff);">
                                        ...
                                        <label class="check-item">
                                            <input type="checkbox" name="classifications[]" value="{{ $cls->id }}"
                                                {{ in_array($cls->id, old('classifications', [])) ? 'checked' : '' }}>
                                            {{ $cls->name }}
                                        </label>
                                        @endforeach
                                    </div>
                                ...
                                <div style="grid-column:1/-1;">
                                    ...
                                    @php $fixedSizes = ['S','M','L','XL','XXL']; $selLabel = old('size_label',''); @endphp
                        ...
                        <div class="form-card card-purple fade-up d4">
                            <div class="section-label">
                                ...
                                <h3>Product Images <span style="color:#f472b6;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:500;">*</span></h3>
                            </div>

                            {{-- Main Image --}}
                            <label class="field-label" style="margin-bottom:10px;">Main Image <span class="req">*</span></label>

                            <div id="main-image-preview" style="display:none;margin-bottom:14px;">
                                <div style="position:relative;display:inline-block;">
                                    <img id="main-preview-img" style="width:120px;height:120px;object-fit:cover;border-radius:16px;border:2px solid #e8d5f0;box-shadow:0 4px 16px rgba(167,139,250,0.2);">
                            ...
                            <div id="main-image-upload" class="upload-area" onclick="document.getElementById('image').click()" style="margin-bottom:24px;">
                                <div style="width:48px;height:48px;border-radius:14px;background:linear-gradient(135deg,#ede9fe,#fce7f3);display:flex;align-items:center;justify-content:center;margin:0 auto 12px;">
                                    <svg class="w-6 h-6" style="color:#a78bfa;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/>
                                    </svg>
                                </div>
                                <p style="font-weight:600;font-size:13.5px;color:#7c3aed;margin-bottom:4px;">Click to upload main image</p>
                                <p style="font-size:11.5px;color:#9ca3af;">PNG, JPG, WEBP · max 2MB</p>
                            ...
                            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
                                <label class="field-label" style="margin:0;">Additional Photos</label>
                                <span style="font-size:11px;color:#9ca3af;background:#f3f4f6;padding:3px 10px;border-radius:99px;">Max 20</span>
                            </div>
                    ...
                    <div class="sticky-sidebar" style="display:flex;flex-direction:column;gap:20px;align-self:start;">
                        ...
                        <div class="form-card card-green fade-up d2">
                            ...
                            <div class="price-wrap" style="margin-bottom:18px;">
                                ...
                                <input type="number" step="0.01" name="prize" id="prize" placeholder="0.00"
                                       class="field-input" style="font-size:18px;font-weight:700;padding-left:42px;"
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
                                <div class="installment-box installment-pink">
                                    ...
                                    <div style="display:flex;align-items:baseline;gap:4px;">
                                        ...
                                        <input type="text" id="installment_3" readonly value="0.00"
                                ...
                                <div class="installment-box installment-blue">
                                    ...
                                    <div style="display:flex;align-items:baseline;gap:4px;">
                                        <span style="font-size:10px;color:#9ca3af;font-weight:600;">Rs</span>
                                        <input type="text" id="installment_4" readonly value="0.00"
...
<div class="form-card card-red fade-up d3">
    ...
    <select name="availability" id="availabilitySelect" class="field-input has-value" required
            onchange="updateAvailabilityPreview()">
        <option value="in stock"     {{ old('availability', 'in stock') == 'in stock'     ? 'selected' : '' }}>✓ In Stock</option>
        <option value="out of stock" {{ old('availability') == 'out of stock' ? 'selected' : '' }}>✗ Out of Stock</option>
    </select>

    <div id="availabilityPreview"
         style="margin-top:12px;padding:10px 14px;border-radius:12px;display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;transition:all 0.25s ease;
         background:rgba(16,185,129,0.08);border:1.5px solid rgba(16,185,129,0.25);color:#059669;">
        <span id="availDot" style="width:8px;height:8px;border-radius:50%;flex-shrink:0;background:#10b981;display:inline-block;"></span>
        <span id="availText">Item will be listed as In Stock</span>
    </div>
</div>
                        ...
                        <div class="form-card card-amber fade-up d3">
                            ...
                            <div id="gift_card_validity_section" style="{{ old('is_gift_card') ? '' : 'display:none;' }} margin-bottom:14px;padding:14px;background:#fff7ed;border-radius:12px;border:1.5px solid #fed7aa;">
                                ...
                                <input type="number" name="gift_card_validity_months" id="gift_card_validity_months"
                                    value="{{ old('gift_card_validity_months') }}" min="1" placeholder="e.g. 12" class="field-input">
                            </div>
                            ...
                            <div id="offer_fields_section" style="{{ old('is_on_offer') ? '' : 'display:none;' }}">
                                    ...
                                    <select name="offer_category_id" id="offer_category_id" class="field-input" onchange="onOfferCategoryChange()">
                                        ...
                                        @foreach($offerCategories as $offerCategory)
                                            <option value="{{ $offerCategory->id }}"
                                                ...
                                                {{ old('offer_category_id') == $offerCategory->id ? 'selected' : '' }}>
                                ...
                                <div style="padding:10px 14px;background:#f0fdf4;border-radius:12px;border:1.5px solid #bbf7d0;margin-bottom:12px;display:flex;align-items:center;gap:10px;">
                                    ...
                                    <span id="offer_discount_badge" style="font-size:20px;font-weight:800;color:#059669;font-family:'DM Sans',sans-serif;">0%</span>
                                </div>
    ...
    <script>
        ...
        /* ══════════════════════════════════════════════════
           ── Select placeholder colour sync ──
           All <select> elements default to muted lavender
           (#c4b5d4) matching "Select colors…", then switch
           to dark (#1f2937) once the user picks a value.
        ══════════════════════════════════════════════════ */
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('select.field-input').forEach(function (sel) {
                ...
                updateColor(); // run immediately — handles old() repopulation on validation failure
            });
        });
        ...
        document.getElementById('add-photo-btn').addEventListener('click', function () {
            if (photoCount >= maxPhotos) { alert('Maximum 20 photos allowed'); return; }
        ...
        function removePhoto(btn) {
            ...
            if (photoCount < maxPhotos) { b.disabled = false; b.style.opacity = '1'; b.style.cursor = 'pointer'; }
        }
        ...
        // Pre-fill old colors after validation failure
        ...
        if (oldColors.length) {
            document.addEventListener('DOMContentLoaded', () => {
                oldColors.forEach(id => {
                    ...
                    if (opt) {
                        selectedColors.push({ id, name: opt.dataset.colorName, hex: opt.dataset.colorHex });
        ...
        /* ── Init on load ── */
        document.addEventListener('DOMContentLoaded', () => {
            if (document.getElementById('is_gift_card').checked) toggleGiftCardValidity();
            if (document.getElementById('is_on_offer').checked)  toggleOfferFields();
            calculateInstallments();
            const categorySelect = document.getElementById('category_id');
            if (categorySelect) {
