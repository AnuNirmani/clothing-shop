<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">
            <span class="bg-gradient-to-r from-pink-400 via-blue-400 to-pink-500 bg-clip-text text-transparent">Materials Management</span>
        </h2>
    </x-slot>
            ...
            <div class="fade-up d1" style="margin-bottom:24px;position:relative;border-radius:24px;overflow:hidden;padding:26px 32px;background:linear-gradient(135deg,#130826,#1e0d4a,#0a1628);box-shadow:0 20px 60px rgba(13,5,32,0.3);">
                ...
                <div style="position:relative;z-index:1;display:flex;align-items:center;gap:18px;">
                    ...
                    <div>
                        ...
                        <h1 style="color:white;font-size:20px;font-weight:700;margin:0;font-family:'Playfair Display',serif;">Materials Management</h1>
                        <p style="color:rgba(255,255,255,0.32);font-size:12px;margin:3px 0 0;font-family:'DM Sans',sans-serif;">Organise and manage your product material catalogue</p>
                    </div>
            ...
            <div class="panel panel-blue fade-up d3">
                <div style="display:flex;align-items:center;justify-content:space-between;padding:18px 24px 14px;">
                    <div style="display:flex;align-items:center;gap:10px;">
                        ...
                        <h3 style="font-family:'Playfair Display',serif;font-size:15px;font-weight:600;color:#1f2937;margin:0;">All Materials</h3>
                    </div>
                ...
                <table>
                    <thead>
                        <tr>
                            <th style="padding-left:24px;">Material Name</th>
                            <th style="padding-right:24px;">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($materials as $material)
                        @php $editError = session('edit_error_id') == $material->id; @endphp
                        <tr>
                            {{-- Name cell --}}
                            <td style="padding-left:24px;">
                                <div id="name-display-{{ $material->id }}" class="name-badge" @if($editError) style="display:none;" @endif>
                                    <span class="name-dot"></span>
                                    {{ $material->name }}
                                </div>

                                <form id="edit-form-{{ $material->id }}"
                                      method="POST" action="{{ route('materials.update', $material->id) }}"
                                      @if(!$editError) style="display:none;" @endif>
                                    @csrf @method('PUT')
                                    <input id="name-input-{{ $material->id }}" type="text" name="name"
                            ...
                            <td style="padding-right:24px;">
                                {{-- Default buttons --}}
                                <div id="action-buttons-{{ $material->id }}"
                                     @if($editError) style="display:none;" @else style="display:flex;justify-content:flex-end;gap:8px;" @endif>
                                    <button type="button" onclick="enableEdit({{ $material->id }})" class="btn-edit">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <form action="{{ route('materials.destroy', $material->id) }}" method="POST" style="display:inline;">
                                        @csrf @method('DELETE')
                                        <button type="submit" onclick="return confirm('Delete this material?')" class="btn-delete">
                                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                {{-- Edit mode buttons --}}
                                <div id="edit-buttons-{{ $material->id }}"
                                     @if($editError) style="display:flex;justify-content:flex-end;gap:8px;" @else style="display:none;" @endif>
                                    <button type="button" onclick="submitEdit({{ $material->id }})" class="btn-save">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/>
                                        </svg>
                                        Save
                                    </button>
                                    <button type="button" onclick="cancelEdit({{ $material->id }})" class="btn-cancel-sm">
                                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <tr>
                            <td colspan="2">
                                <div class="empty-state">
                                    ...
                                    <p style="font-family:'Playfair Display',serif;font-size:16px;font-weight:600;color:#374151;margin-bottom:5px;">No materials yet</p>
                                    <p style="font-size:12.5px;color:#9ca3af;">Add your first material using the form above</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
                ...
                <div style="padding:16px 24px;background:linear-gradient(135deg,#fdf9ff,#f0f9ff);border-top:1px solid #f3e8ff;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
                    <p style="font-size:12.5px;color:#6b7280;margin:0;">
                        Showing
                        <span style="font-weight:700;color:#be185d;">{{ $materials->firstItem() ?? 0 }}</span>–<span style="font-weight:700;color:#be185d;">{{ $materials->lastItem() ?? 0 }}</span>
                        of
                        <span style="font-weight:700;color:#3b82f6;">{{ $materials->total() }}</span>
                        materials
                    </p>
                    <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
                        ...
                        @else
                            <a href="{{ $materials->previousPageUrl() }}" class="page-btn">← Prev</a>
                        @endif
                        ...
                        @if($materials->hasMorePages())
                            <a href="{{ $materials->nextPageUrl() }}" class="page-btn">Next →</a>
                        @else
