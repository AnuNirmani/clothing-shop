                        ...
                        <tbody class="divide-y">
                            @forelse($materials as $material)
                                ...
                                <tr class="hover:bg-gradient-to-r hover:from-pink-50 hover:to-blue-50">
                                    ...
                                    <td class="px-6 py-4">
                                        <div
                                            id="action-buttons-{{ $material->id }}"
                                            @if($editError) style="display:none" @endif
                                            class="flex justify-center gap-3"
                                        >
                                            <button
                                                type="button"
                                                    onclick="enableEdit({{ $material->id }})"
                                        ...
                                        <div
                                            id="edit-buttons-{{ $material->id }}"
                                            @if($editError) style="display:flex" @else class="hidden" @endif
                                            class="flex justify-center gap-3"
                                        >
                                            <button
                                                type="button"
                                                onclick="submitEdit({{ $material->id }})"
                ...
                <div class="px-6 py-4 bg-gradient-to-r from-pink-50 via-blue-50 to-pink-50 border-t border-pink-100">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-600">
                            Showing <span class="font-semibold text-pink-600">{{ $materials->firstItem() ?? 0 }}</span> to <span class="font-semibold text-pink-600">{{ $materials->lastItem() ?? 0 }}</span> of <span class="font-semibold text-blue-600">{{ $materials->total() }}</span> results
                        </div>
                        <div class="flex space-x-1">
                            ...
                            @else
                                <a href="{{ $materials->previousPageUrl() }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-pink-200 rounded-lg hover:bg-gradient-to-r hover:from-pink-100 hover:to-blue-100 hover:border-pink-300 transition-all duration-200">Previous</a>
                            @endif
                            ...
                            @if ($materials->hasMorePages())
                                <a href="{{ $materials->nextPageUrl() }}" class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-blue-200 rounded-lg hover:bg-gradient-to-r hover:from-pink-100 hover:to-blue-100 hover:border-blue-300 transition-all duration-200">Next</a>
                            @else
